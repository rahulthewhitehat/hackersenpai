...
class _WindowsVideoPlaylistScreenState extends State<WindowsVideoPlaylistScreen> with WidgetsBindingObserver {
  ...
  void didChangeMetrics() {
    ...
    if (_isInitialized && _currentVideo != null) {
      // Detect full-screen by checking if the window size matches the screen size
      final size = MediaQuery.of(context).size;
      final isFullScreen = size.width >= WidgetsBinding.instance.window.physicalSize.width / WidgetsBinding.instance.window.devicePixelRatio;
  ...
  Future<void> _changeVideo(VideoModel video) async {
    if (_currentVideo?.id == video.id && _currentVideoId == video.videoId) return;
  ...
  Widget build(BuildContext context) {
    ...
    final playerWithWatermark = _buildVideoPlayerWithWatermark(player, watermarkText);
